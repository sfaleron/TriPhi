 ...
 * Any other flags will be ignored, but it is an error if neither
 * are present.
 ...
 * Insufficiently documented behavior, works on my system, seems
 * likely to be more or less canonical, but you should check with
 * the node maintainers about that!
...
const validateFile = (file, shortDesc, flags, callback) => {
  ...
  const fmtCb = (msg, err) => {
    callback(`| ${shortDesc} | ${msg}${err ? ": " + err.message : ""}`);
  }
